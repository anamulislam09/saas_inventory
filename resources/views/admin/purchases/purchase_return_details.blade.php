            ...
            <div id="invoice-top">
                ...
                <div class="title">
                    <h1 class="h1">Vouchar #<span
                            class="invoiceVal invoice_num">{{ $data['purchaseReturn']->vouchar_no }}</span></h1>
                    <p class="p">Vouchar Date: <span
                            id="invoice_date">{{ date('dS M Y', strtotime($data['purchaseReturn']->date)) }}</span></p>
                ...
                <div class="clearfix">
                    <div class="col-left">
                        ...
                        <div class="clientinfo">
                            <h2 class="h2" id="vendor">{{ $data['purchaseReturn']->supplier->name }},
                                {{ $data['purchaseReturn']->supplier->organization }}</h2>
                            <p class="p"><span
                                    id="address">{{ $data['purchaseReturn']->supplier->address }}</span><br><span
                                    id="city"></span><span id="country"></span><span id="zip"></span><span
                                    id="tax_num">{{ $data['purchaseReturn']->supplier->phone }},
                                    {{ $data['purchaseReturn']->supplier->email }}</span><br></p>
                        </div>
                    </div>
                    <div class="col-right">
                        <table class="table">
                            <tbody>
                                <tr>
                                    <td><span>Vouchar Total</span><label
                                            ...
                                            {{ number_format($data['purchaseReturn']->total_return_amount, 2) }}</label>
                                    </td>
                                ...
                                <tr>
                                    <td colspan="2"><span>Note</span>:<label
                                            id="note">{{ $data['purchaseReturn']->note }}</label></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
            ...
            <div class="invoice-bot">
                <div class="table-2">
                    <table class="table-main">
                        ...
                        <tbody>
                            @foreach ($data['purchaseReturn']->purchase_return_details as $key => $value)
                                <tr class="list-item">
                                    <td>{{ $key + 1 }}</td>
                            ...
                            <tr class="list-item total-row">
                                ...
                                <td style="text-align: right;">{{ $data['basicInfo']->currency_symbol }}
                                    {{ number_format($data['purchaseReturn']->total_return_amount, 2) }}</td>
                            </tr>
                            <tr class="list-item total-row">
                                <th colspan="4">Paid Amount</th>
                                <td style="text-align: right;">{{ $data['basicInfo']->currency_symbol }}
                                    {{ number_format($data['purchaseReturn']->refund_amount, 2) }}</td>
                            </tr>
                            <tr class="list-item total-row">
                                <th colspan="4">Due Amount</th>
                                <td style="text-align: right;">{{ $data['basicInfo']->currency_symbol }}
                                    {{ number_format($data['purchaseReturn']->total_return_amount - $data['purchaseReturn']->refund_amount, 2) }}
                                </td>
                            </tr>
                        </tbody>
                    </table>
                    ...
                    <table class="table-main">
                        ...
                        <tbody>
                            @php
                                $total_price = 0;
                            @endphp
                            @foreach ($data['purchaseReturn']->payments as $key => $value)
                                <tr class="list-item">
...
@section('script')
    ...
    <script type="text/javascript">
        JsBarcode(".barcode", "{{ $data['purchaseReturn']->vouchar_no }}", {
            // format: "upc",
            // lineColor: "#0aa",
            width: 1,
            height: 30,
